...
*   Fit a linear regression model on each sample with `tmax` as the response
and `tmin` as the predictor.
*   Estimate $r^2$ and estimate log($\beta_0 * \beta_1$) for each fitted model.
We will use natural log here.
...
Both estimates appear roughly normally distributed. However, the peak of each
distribution appears a little less normal. The R-Squared distribution
appears to be a little left skewed.

We can provide a point estimate of the r-squared and the natural log of the 
product of the beta coefficients by averaging the estimates over all 
bootstrapped samples. Additionally, we can provide a 95% confidence interval for 
each point estimate by getting the 2.5% and 97.5% quantiles.
```{r}
